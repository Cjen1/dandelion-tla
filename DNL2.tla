...
VARIABLES
  ...
  M_read_resp,
  \* @type: Set({tid : $tid, txn : $txn, deps : $dependencies, is_ser: Bool});
  M_lock,
...
/\ \E R \in [Txns[t].read -> {m \in M_read_resp: m.tid = t}]: 
   ...
   /\ M_lock' = M_lock \union 
        {[tid |-> t, txn |-> Txns[t], deps |-> read_deps, is_ser |-> FALSE]}
...
/\ IsPrimary[r]
/\ ~\E nm \in M_lock_resp: nm.src = r /\ nm.tid = m.tid /\ nm.ser = m.is_ser
/\ IF /\ Replicas[r].log.tag = "unlocked"
   ...
   THEN /\ Replicas' = [Replicas EXCEPT ![r] = [
        ...
        /\ M_lock_resp' = M_lock_resp \union 
             {[src |-> r, tid |-> m.tid, version |-> Replicas[r].version, locked |-> TRUE, ser |-> m.is_ser]}
   ELSE /\ UNCHANGED Replicas
        /\ M_lock_resp' = M_lock_resp \union 
             {[src |-> r, tid |-> m.tid, version |-> Replicas[r].version, locked |-> FALSE, ser |-> m.is_ser]}
...
LET responses == {m \in M_lock_resp: m.tid = t}
    ...
    prev_write == [ 
      k \in Txns[t].write |-> 
        (CHOOSE m \in responses: KeyLookup[m.src] = k).version]
    new_deps == prev_write @@ Txn_deps[t] IN
...
/\ IF \A m \in responses: m.locked = TRUE
   THEN /\ Coordinator_state' = [Coordinator_state EXCEPT ![t] = "Unlock"]
        /\ M_lock' = M_lock \union {[tid |-> t, txn |-> Txns[t], deps |-> new_deps, is_ser |-> TRUE]}
        /\ UNCHANGED << M_unlock >>
...
ReplicaReplicate(r) ==
\E m \in M_lock:
/\ KeyLookup[r] \in m.txn.write /\ m.is_ser
/\ \/ IsBackup[r] 
...
LET ser_responses == {m \in M_lock_resp: m.tid = t /\ m.ser} 
    ...
    relevant == UNION {Shards[k] : k \in Txns[t].write} 
    backups == {r \in relevant: IsBackup[r]}
IN
/\ Coordinator_state[t] = "Unlock"
\* Response from every backup
/\ backups \subseteq ser_replicas
\* Exists a shard which every replica is serialised
/\ \E s \in Txns[t].write: Shards[s] \subseteq ser_replicas
